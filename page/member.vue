<template>
	<div>
        ...
        <el-form-item label="是否Vip：">
          <el-select v-model="serachForm.isVip" placeholder="请选择" class="input-small">
            <el-option
            v-for="item in vips"
            :key="item.value"
            :label="item.label"
...
<el-table :data="tableData" border  style="width: 100%">
  ...
  <el-table-column align="center" prop="Nickname" label="昵称" min-width="300"></el-table-column>
  <el-table-column align="center" prop="OpenId" label="OpenId" min-width="300"></el-table-column>


  <el-table-column align="center" prop="Mobile" label="手机号" width="100">
  </el-table-column>
  <el-table-column align="center" prop="Balance" label="余额" width="100">
  </el-table-column>
  <el-table-column align="center" prop="IsVip" label="是否vip" width="100">
    <template slot-scope="props">{{props.row.IsVip==1?"是":"否"}}</template>
  </el-table-column>
  <el-table-column align="center" prop="Status" label="状态" width="50">
   <template slot-scope="props">{{props.row.Status==1?"有效":"无效"}}</template>
 </el-table-column>
 <el-table-column align="center" prop="VipStarttime" label="vip开始时间" min-width="180">
   <template slot-scope="props">
    {{new Date(props.row.VipStarttime).ljyFormat("yyyy-MM-dd HH:mm")}}
  </template>
</el-table-column>
<el-table-column align="center" prop="VipEndtime" label="vip结束时间" min-width="180">
 <template slot-scope="props">
  {{new Date(props.row.VipEndtime).ljyFormat("yyyy-MM-dd HH:mm")}}
</template>
</el-table-column>
	...
	data() {
     ...
     methods: {
      ...
      search(){  // 查询函数
      	...
      	console.log(para);
      	http.httpPost("/api/v1/manager/user/GetUserList",para).then(data=>{
