<template>
	<div>
		<v-tab></v-tab>
		<v-search searchName="查询评论">
			<template>
				<el-form label-width="100px" label-position="right" :inline="true" :model="serachForm">
					<el-form-item label="课程名：">
						<el-input v-model="serachForm.collageName" placeholder="请输入课程名" 
...
methods: {
	onSubmit(e) {
      ...
      search(){  // 查询函数
      	...
      	console.log(para);
      	http.httpPost("/api/v1/manager/collageEvaluate/GetCollageEvaluateList",para).then(data=>{
